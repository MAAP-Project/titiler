    ...
    def mosaics(self):  # noqa: C901
        ...
        async def get_mosaic(request: Request, mosaic_id: str) -> MosaicEntity:
            ...
            if await retrieve(mosaic_id):
                ...
                raise HTTPException(
                    HTTP_404_NOT_FOUND, f"Error: mosaic with given ID does not exist."
                )
        ...
        async def post_mosaics(
            ...
            try:
                ...
                raise HTTPException(
                    HTTP_409_CONFLICT, f"Error: mosaic with given ID already exists"
                )
            except Exception as e:
                logging.error(f"could not save mosaic: {e}")
                raise HTTPException(
                    HTTP_500_INTERNAL_SERVER_ERROR, f"Error: could not save mosaic"
                )
        ...
        async def put_mosaic(
            ...
            try:
                ...
                raise HTTPException(
                    HTTP_500_INTERNAL_SERVER_ERROR, f"Error: could not update mosaic."
                )
        ...
        async def mosaicjson_from_urls(urisrb: UrisRequestBody) -> MosaicJSON:
            ...
            if mosaicjson is None:
                raise HTTPException(
                    HTTP_500_INTERNAL_SERVER_ERROR,
                    f"Error: could not extract mosaic data",
                )
        ...
        async def mosaicjson_from_stac_api_query(
            ...
            if not req.stac_api_root:
                raise HTTPException(
                    HTTP_400_BAD_REQUEST,
                    f"Error: stac_api_root field must be non-empty.",
                )
        ...
        def extract_mosaicjson_from_features(
            ...
            if features:
                ...
                # https://github.com/mapbox/supermercado/issues/47
                    ...
                    raise Exception(
                        f"STAC Items likely have MultiPolygon geometry, and only Polygon is supported."
                    )
                except Exception as e:
